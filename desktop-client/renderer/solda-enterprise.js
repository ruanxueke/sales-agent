...
(function () {
  ...
  async function loadAll() {
    ...
    try {
      const [tenants, billingHealth, plans, funnel, roi, aiImpact, forecast, aiStatus, voice, integrations, webhook, org, webchatSessions, alertRules, alertEvents, billingPlans, billingInvoices, webhookList] = await Promise.all([
        getJSON("http://127.0.0.1:5000/api/v1/tenants").catch(() => ({ items: [] })),
        getJSON("http://127.0.0.1:5000/api/v1/billing/health").catch(() => ({})),
        getJSON("http://127.0.0.1:5000/api/v1/billing/plans").catch(() => ({ plans: {} })),
        getJSON("http://127.0.0.1:5000/api/v1/analytics/funnel?days=30").catch(() => ({ steps: [] })),
        getJSON("http://127.0.0.1:5000/api/v1/analytics/channel-roi?days=30").catch(() => ({ items: [] })),
        getJSON("http://127.0.0.1:5000/api/v1/analytics/ai-impact?days=30").catch(() => ({ channels: {} })),
        getJSON("http://127.0.0.1:5000/api/v1/forecast/list?limit=10").catch(() => ({ items: [] })),
        getJSON("http://127.0.0.1:5000/api/v1/multimodal/status").catch(() => ({})),
        getJSON("http://127.0.0.1:5000/api/v1/voice/status").catch(() => ({})),
        getJSON("http://127.0.0.1:5000/api/v1/integrations/status").catch(() => ({})),
        getJSON("http://127.0.0.1:5000/api/v1/webhooks").catch(() => ({ items: [] })),
      getJSON("http://127.0.0.1:5000/api/v1/org/overview").catch(() => ({ items: [] })),
      getJSON("http://127.0.0.1:5000/api/v1/webchat/sessions").catch(() => ({ items: [] })),
      getJSON("http://127.0.0.1:5000/api/v1/alerting/rules").catch(() => ({ items: [] })),
      getJSON("http://127.0.0.1:5000/api/v1/alerting/events?limit=20").catch(() => ({ items: [] })),
      getJSON("http://127.0.0.1:5000/api/v1/billing/plans").catch(() => ({ plans: {} })),
      getJSON("http://127.0.0.1:5000/api/v1/billing/invoices").catch(() => ({ items: [] })),
      getJSON("http://127.0.0.1:5000/api/v1/webhooks").catch(() => ({ items: [] })),
      ]);
  ...
  function applyAccess() {
    ...
    if (!token) return;
    getJSON("http://127.0.0.1:5000/api/v1/accounts/tenant-me").then((d) => {
      const role = d.role || "";
  ...
  function bind() {
    ...
    document.addEventListener("click", async (e) => {
      if (e.target && e.target.id === "org-create") {
        ...
        try {
          await postJSON("http://127.0.0.1:5000/api/v1/tenants", { name });
          await loadAll();
      ...
      if (e.target && e.target.id === "bot-check-toggle") {
        ...
        try {
          const data = await getJSON("http://127.0.0.1:5000/api/v1/alerting/rules");
          ...
          await postJSON("http://127.0.0.1:5000/api/v1/alerting/rules", { rule_id: rule.id, rule_type: "bot_offline", name: rule.name, enabled: next, threshold: rule.threshold, window_seconds: rule.window_seconds, channels: rule.channels, receivers: rule.receivers, cooldown_seconds: rule.cooldown_seconds });
      ...
      if (e.target && e.target.id === "alert-test") {
        ...
        try {
          const d = await postJSON("http://127.0.0.1:5000/api/v1/alerting/test", { title: "测试告警", content: "这是一条测试告警，确认通知通道正常。" });
      ...
      if (e.target && e.target.id === "biz-subscribe" || (e.target && e.target.dataset && e.target.dataset.subscribe)) {
        ...
        try {
          const d = await postJSON("http://127.0.0.1:5000/api/v1/billing/subscribe", { tenant_id: tenantId, plan, days: 30 });
      ...
      if (e.target && e.target.id === "hook-add") {
        ...
        try {
          await postJSON("http://127.0.0.1:5000/api/v1/webhooks", { url, events: ["customer.created", "order.paid", "chat.received"] });
      ...
      if (e.target && e.target.id === "org-member-add") {
        ...
        try {
          await postJSON("http://127.0.0.1:5000/api/v1/org/members", { tenant_id: tenantId, username, password, name, role });
          await loadAll();
