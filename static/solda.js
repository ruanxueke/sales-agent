...
async function loadAmmo() {
  const [products, objections, competitors, scripts, cases] = await Promise.all([
    fetchJSON("/api/v1/ammo/products"),
    fetchJSON("/api/v1/ammo/objections"),
    fetchJSON("/api/v1/ammo/competitors"),
    fetchJSON("/api/v1/ammo/scripts"),
    fetchJSON("/api/v1/ammo/cases"),
  ]);
  state.ammo = {
...
async function loadWinning() {
  const [differentiators, evidence, tools] = await Promise.all([
    fetchJSON("/api/v1/winning/differentiators"),
    fetchJSON("/api/v1/winning/evidence"),
    fetchJSON("/api/v1/winning/tools"),
  ]);
  state.winning = {
...
async function loadEngagement() {
  const [appointments, coupons, userCoupons, paymentLinks, stats] = await Promise.all([
    ...
    fetchJSON("/api/v1/coupons"),
    fetchJSON("/api/v1/user-coupons"),
    fetchJSON("/api/v1/payment-links"),
    fetchJSON("/api/v1/engagement/stats"),
  ]);
  state.engagement = {
...
async function loadOptimization() {
  try {
    const [scores, scoreStats, prompts, experiments, suggestions] = await Promise.all([
      fetchJSON("/api/v1/optimize/scores?limit=20"),
      fetchJSON("/api/v1/optimize/scores/stats?days=7"),
      fetchJSON("/api/v1/optimize/prompts"),
      fetchJSON("/api/v1/optimize/experiments"),
      fetchJSON("/api/v1/optimize/suggestions"),
    ]);
    state.selfOpt = {
...
async function activatePrompt(id) {
  await fetchJSON("/api/v1/optimize/prompts/" + id + "/activate", { method: "POST" });
  toast("已激活");
  loadOptimization();
}

async function startExperiment(id) {
  await fetchJSON("/api/v1/optimize/experiments/" + id + "/start", { method: "POST" });
  toast("实验已启动");
  loadOptimization();
}

async function viewExpResults(id) {
  const r = await fetchJSON("/api/v1/optimize/experiments/" + id + "/results");
...
async function approveSuggestion(id) {
  await fetchJSON("/api/v1/optimize/suggestions/" + id + "/approve", { method: "POST" });
  toast("已批准");
  loadOptimization();
}

async function rejectSuggestion(id) {
  await fetchJSON("/api/v1/optimize/suggestions/" + id + "/reject", { method: "POST" });
  toast("已拒绝");
  loadOptimization();
}
async function loadCompliance() {
  const [rules, stats] = await Promise.all([
    fetchJSON("/api/v1/compliance/rules"),
    fetchJSON("/api/v1/compliance/stats"),
  ]);
...
/* ===== 编辑/删除注册 ===== */
registerDelete("ammo-products", "/api/v1/ammo/products", loadAmmo);
registerDelete("ammo-objections", "/api/v1/ammo/objections", loadAmmo);
registerDelete("ammo-competitors", "/api/v1/ammo/competitors", loadAmmo);
registerDelete("ammo-scripts", "/api/v1/ammo/scripts", loadAmmo);
registerDelete("ammo-cases", "/api/v1/ammo/cases", loadAmmo);
registerDelete("winning-df", "/api/v1/winning/differentiators", loadWinning);
registerDelete("winning-ev", "/api/v1/winning/evidence", loadWinning);
registerDelete("winning-tools", "/api/v1/winning/tools", loadWinning);
registerDelete("engagement-app", "/api/v1/appointments", loadEngagement);
registerDelete("engagement-coupon", "/api/v1/coupons", loadEngagement);
registerDelete("engagement-uc", "/api/v1/user-coupons", loadEngagement);
registerDelete("engagement-pl", "/api/v1/payment-links", loadEngagement);
registerDelete("optimization-exp", "/api/v1/optimize/experiments", loadOptimization);
registerDelete("optimization-review", "/api/v1/optimize/reviews", loadOptimization);
registerDelete("compliance-rules", "/api/v1/compliance/rules", loadCompliance);

registerEdit("ammo-products", "/api/v1/ammo/products", [{key:"product_name",label:"产品名"}, {key:"price",label:"价格"}, {key:"selling_points",label:"卖点"}], loadAmmo);
registerEdit("ammo-objections", "/api/v1/ammo/objections", [{key:"trigger_scene",label:"异议场景"}, {key:"standard_reply",label:"标准回复"}], loadAmmo);
registerEdit("ammo-competitors", "/api/v1/ammo/competitors", [{key:"competitor",label:"竞品"}, {key:"standard_reply",label:"标准回应"}], loadAmmo);
registerEdit("ammo-scripts", "/api/v1/ammo/scripts", [{key:"followup_scene",label:"状态"}, {key:"script_example",label:"话术"}], loadAmmo);
registerEdit("winning-df", "/api/v1/winning/differentiators", [{key:"differentiator",label:"差异点"}, {key:"evidence",label:"证据"}], loadWinning);
registerEdit("winning-ev", "/api/v1/winning/evidence", [{key:"title",label:"标题"}, {key:"content",label:"内容"}], loadWinning);
registerEdit("winning-tools", "/api/v1/winning/tools", [{key:"name",label:"工具名"}, {key:"description",label:"说明"}], loadWinning);
registerEdit("engagement-app", "/api/v1/appointments", [{key:"title",label:"标题"}, {key:"start_at",label:"开始时间"}, {key:"owner",label:"负责人"}], loadEngagement);
registerEdit("engagement-coupon", "/api/v1/coupons", [{key:"name",label:"券名"}, {key:"value",label:"券值"}], loadEngagement);
registerEdit("engagement-pl", "/api/v1/payment-links", [{key:"title",label:"标题"}, {key:"link",label:"链接"}], loadEngagement);
registerEdit("optimization-exp", "/api/v1/optimize/experiments", [{key:"name",label:"实验名"},{key:"control",label:"对照组"},{key:"variant",label:"实验组"}], loadOptimization);
registerEdit("compliance-rules", "/api/v1/compliance/rules", [{key:"description",label:"说明"}], loadCompliance);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("pk-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/ammo/products", { fields: { product_name: $("pk-name").value.trim(), price: $("pk-price").value.trim(), selling_points: $("pk-selling").value.trim() } });
  ...
  $("obj-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/ammo/objections", { fields: { trigger_scene: $("obj-scene").value.trim(), standard_reply: $("obj-reply").value.trim(), evidence: $("obj-evidence").value.trim(), next_step: $("obj-next").value.trim() } });
  ...
  $("cp-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/ammo/competitors", { fields: { competitor: $("cp-name").value.trim(), customer_saying: $("cp-saying").value.trim(), differentiator_1: $("cp-diff1").value.trim(), standard_reply: $("cp-reply").value.trim() } });
  ...
  $("fs-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/ammo/scripts", { fields: { followup_scene: $("fs-scene").value, script_example: $("fs-script").value.trim(), value_point: $("fs-value").value.trim(), next_goal: $("fs-goal").value.trim(), delay_hours: Number($("fs-delay").value || 24) } });
  ...
  $("case-import").addEventListener("click", async () => {
    ...
    try {
      const data = await postJSON("/api/v1/ammo/cases/import", { content, title: $("case-title").value.trim(), channel: $("case-channel").value.trim() });
  ...
  $("df-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/winning/differentiators", { fields: { product: $("df-product").value.trim(), differentiator: $("df-point").value.trim(), customer_concern: $("df-concern").value.trim(), evidence: $("df-evidence").value.trim() } });
  ...
  $("ev-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/winning/evidence", { fields: { evidence_type: $("ev-type").value, title: $("ev-title").value.trim(), content: $("ev-content").value.trim(), link: $("ev-link").value.trim(), scenario: $("ev-scenario").value.trim() } });
  ...
  $("tool-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/winning/tools", { fields: { tool_type: $("tool-type").value, name: $("tool-name").value.trim(), description: $("tool-desc").value.trim(), trigger_scene: $("tool-scene").value.trim() } });
  ...
  $("app-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/appointments", { fields: { session_id: $("app-session").value.trim(), title: $("app-title").value.trim(), start_at: $("app-start").value.trim(), owner: $("app-owner").value.trim(), appointment_type: "trial" } });
  ...
  $("coupon-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/coupons", { fields: { name: $("coupon-name").value.trim(), coupon_type: $("coupon-type").value, value: $("coupon-value").value.trim(), valid_days: Number($("coupon-days").value || 7) } });
  ...
  $("pl-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/payment-links", { order_id: Number($("pl-order").value || 0), title: $("pl-title").value.trim(), amount: Number($("pl-amount").value || 0), link: $("pl-link").value.trim() });
  ...
  $("exp-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/optimize/experiments", { fields: { name: $("exp-name").value.trim(), kind: $("exp-kind").value || "prompt", control: $("exp-control").value.trim(), variant: $("exp-variant").value.trim() } });
  ...
  $("review-add").addEventListener("click", async () => {
    ...
    if (!content) return;
    try {
      await postJSON("/api/v1/optimize/reviews", { fields: { content } });
  ...
  $("comp-add").addEventListener("click", async () => {
    try {
      await postJSON("/api/v1/compliance/rules", { rule_type: $("comp-type").value, trigger_words: $("comp-words").value.trim(), action: $("comp-action").value, description: $("comp-desc").value.trim() });
